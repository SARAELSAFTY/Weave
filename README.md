...
- A configured LLM proxy URL (Cloudflare Worker included in `/proxy`)

The project remains usable without a configured proxy: authored cards and resource logic continue to work, while LLM-dependent cards follow their configured fallback behavior.
...
4. Configure the `LlmReactionClient` on `GameManager` with a `LlmSettings` asset and your `Proxy URL`.
5. Enter Play Mode.
...
3. Connect cards through their left and right ports, and apply resource changes to either choice.
...
| --- | --- | --- |
| Branching card | Left/right choices and outgoing links | Applies the selected resource change, then follows the selected link. |
...
All in-game LLM requests route through `LlmReactionClient` to the Cloudflare Worker proxy (`/proxy`). 

- **Security & Secret Isolation**: The game client (desktop or WebGL) never holds or transmits the Groq API key. The proxy attaches the `GROQ_API_KEY` secret server-side and forwards requests to Groq.
- **Abuse Protection & CORS**: The proxy handles CORS preflights and enforces method restrictions (`POST` only) and body size limits (≤20KB).
...
Use **Weave > LLM Tester** to preview composed prompts and test live generations with your configured proxy.
...
`-- proxy/                      Cloudflare Worker proxy source and configuration
```
